...
Status: proposal (2026-08-18). Nothing built yet. This document is the reference for the build;
...
- Every source type enters through the same `-i` (FFmpeg 9 handles GIF, animated WebP, APNG, MP4/MKV/MOV/WebM, PNG sequences via concat demuxer).
- **AVIF ingest:** ffmpeg's mov demuxer exposes AVIF alpha as a *second* stream (`[0:v:0][0:v:1]alphamerge`); for animated AVIF this is unverified → default to `avifdec --index all` → RGBA PNGs *(verify)*.
...
- Duplicate-frame merge: `mpdecimate` **with `-fps_mode vfr`** (otherwise ffmpeg re-duplicates the dropped frames). Image-sequence per-frame delays go through the concat demuxer with `-fps_mode passthrough`.
...
| Sticker | **524,288 B** (512 KiB), **exactly 320×320**, ≤ 5 s, ≤ 1000 frames, ≤ 60 fps | PNG static; **APNG or GIF** animated (GIF since Jan 2023; Lottie verified/partnered only). **WebP is not a sticker format.** | Rendered at 160×160 dp. Zero/odd delays → "frame rate too small or too large". APNG only animates when uploaded under Server Settings › Stickers; **APNG chat attachments show frame 0 only** (lilliput strips APNG frames, open since 2022). GIF sticker upload via the client UI: *(verify)*. |
...
**APNG (stickers only)**
- `acTL num_plays = 0`; first frame is a real fcTL frame; all fcTL rects inside 320×320; delays ≥ 20 ms and never 0; ≤ 5.0 s; ≤ 1000 frames; ≤ 60 fps; ≤ 524,288 B. UI: "APNG animates only as a server sticker, not as a chat attachment; never for emoji."
...
| Sticker (animated) | exactly 320×320 | 524,288 B (aim ≤ 515,000), ≤ 5 s | RGBA APNG probe → **indexed 8-bit-alpha APNG** → GIF (`--lossy` up to 200) | (25,256) → (20,256) → (16.7,256) → (12.5,128) → (10,64); helpers: "speed up to fit 5 s" (`setpts=PTS/K`), "skip leading blank frames" |
...
2. **Discord presets + optimisation.** Emote/Sticker/Chat presets, fit engine (parallel ladder + secant + alternatives + binding knob), APNG (RGBA + indexed 8-bit alpha) + lint, static path (pngquant/oxipng/cwebp/avifenc), GIF→GIF gifsicle-only optimiser, extract frames (zip + grid), image sequence → GIF/WebP/APNG/AVIF, AVIF via avifenc/avifdec, in-chat thumbnails, result memoisation, sweeper.
