<script lang="ts">
  ...
  const summary = $derived(cfg.enabled ? `${fmtNum(cfg.fps)} fps` : `source ${fmtNum(info.fps)} fps`);
...
<OpCard title="Frame rate" {summary} bind:enabled={app.ops.fps.enabled} bind:open>
  ...
  <p class="hint">
    Source: {fmtNum(info.fps)} fps. This op wins over the Output fps.
    {#if format === 'gif'}
      {#if capped}
      ...
      {/if}
    {:else if capped}
      WebP is capped at <b>{MAX_FPS} fps</b>.
    {:else}
      WebP keeps {fmtNum(cfg.fps)} fps exactly (cap {MAX_FPS}).
    {/if}
