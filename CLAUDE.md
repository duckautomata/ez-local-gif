...
- `internal/discordlint` — GIF/WebP/APNG/static byte-level lint + fix, `MergeGIFHolds` (folds harmless no-op GIF frames into the previous frame's delay — jobs runs it on ffmpeg's GIF output before the gifsicle post-pass and inside the hold repair), plus Phase 4 structural MP4/WebM lint (`LintVideo`, `video.*` rules, no fixer); stdlib only
